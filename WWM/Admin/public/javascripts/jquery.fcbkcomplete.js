...
(function( $, undefined ) {
  $.fn.fcbkcomplete = function(opt) {
    return this.queue( function() {
      ...
      function createFCBK() {
        holder = $('<ul class="holder ui-sortable"></ul>').width(options.width);
        if (options.attachto) {
      ...
      function addItem(title, value, preadded, locked, focusme, newel) {
        if (!maxItems()) {
          return false;
        }
        ...
        if (!preadded) {
          ...
          addInput(focusme);
		if(newel=='new'){
          var _item = $('<option value="'+xssDisplay(value, 1)+'" id="opt_'+id+'" class="selected"  data-item="'+newel+'">'+xssDisplay(title)+'</option>');
          element.append(_item);
         }else{
         	var _item = $('<option value="'+xssDisplay(value, 1)+'" id="opt_'+id+'" class="selected" selected="selected" data-item="'+newel+'">'+xssDisplay(title)+'</option>');
          element.append(_item);
         }
		...
		var cur_val = $('#'+select_id+'_new').val();

			if(cur_val){
            
			$('#'+select_id+'_new').val(cur_val + "," +value);
			}
			else{
			
			$('#'+select_id+'_new').val(value);
			}
		}
      ...
      function removeItem(item) {
        if (!item.hasClass('locked')) {
          ...
          if (id) {
             $("#o" + id + "").remove();
			 var compon = [];
			 $("#"+element.attr('id')+" option[data-item='new']").each(function(){compon.push($(this).attr('value'));$("#"+element.attr('id')+"_new").val(compon.join(','));
});
          } else {
      ...
      function bindEvents() {
        ...
        feed.children("li").unbind("mousedown").mousedown( function() {
          var option = $(this);
		  var item="old";
          addItem(option.text(), option.attr("rel"), 0, 0, 1, item);
          clear_feed(1);
          complete.hide();
        });
         ...
         if ((event.keyCode == _key.enter || event.keyCode == _key.tab || event.keyCode == _key.comma) && checkFocusOn()) {
          ...
          if ((event.keyCode == _key.enter || event.keyCode == _key.tab || event.keyCode == _key.comma) && !checkFocusOn()) {
            if (options.newel) {
			  ...
			  var item="new";
              addItem(value, value, 0, 0, 1, item);
              return _preventDefault(event);
            }
            if ((options.addontab || options.addoncomma) && options.newel) {
              ...
              addItem(option.text(), option.attr("rel"), 0, 0, 1, 0);
              return _preventDefault(event);
            }
          }
