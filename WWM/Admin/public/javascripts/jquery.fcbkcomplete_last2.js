    ...
    return this.queue( function() {
      ...
      function elPrepare() {
        ...
        $(element).bind("addItem", function(event, data) {
          addItem(data.title, data.value, 0, 0, 0,0);
        });
      ...
      function addItem(title, value, preadded, locked, focusme, newitem) {
        if (!maxItems()) {
          return false;
        }
        var liclass = "bit-box" + (locked ? " locked": "");
		var new_item= (newitem ? " newitem": "");
        var id = randomId();
        var txt = document.createTextNode(xssDisplay(title));
        var aclose = $('<a class="closebutton" href="#"></a>');
        var li = $('<li class="'+liclass+'" rel="'+value+'" id="pt_'+id+'" name="'+new_item+'"></li>').prepend(txt).append(aclose);
      ...
      function bindEvents() {
        ...
        maininput.keydown( function(event) {
          ...
          if ((event.keyCode == _key.enter || event.keyCode == _key.tab || event.keyCode == _key.comma) && checkFocusOn()) {
            var option = focuson;
            addItem(option.text(), option.attr("rel"), 0, 0, 1,1);
