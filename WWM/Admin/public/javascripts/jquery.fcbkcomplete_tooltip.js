...
(function( $, undefined ) {
  $.fn.fcbkcomplete = function(opt) {
    return this.queue( function() {
      ...
      function elPrepare() {
	    ...
	    if(options.tooltip==true){
	      var id = addItem(option.text(), option.val(),option.attr("title"), true, option.hasClass("locked"));
	      temp_elem.append('<option title="'+option.attr("title")+'" value="'+option.val()+'" selected="selected" id="opt_'+id+'"class="selected">'+option.text()+'</option>');
	    }
	    else{
	...
	if(options.tooltip==true){
	  tooltip_hover();
	  var li = $('<li title="'+tip+'" class="'+liclass+'" rel="'+value+'" id="pt_'+id+'"></li>').prepend(txt).append(aclose);
	  var create_title = $('<span class="title_display">'+tip+'</span>');
	  $(aclose).after(create_title);
	  //tooltip_hover();
	}
	else{
      ...
      function tooltip_hover(){
	var holder_li = $(holder).find('li');
	if(options.tooltip){
	  holder_li.mouseover(function(){
	    $(this).css('cursor','pointer');
	    $(this).find('span').css("display","block");
	  });
	  holder_li.mouseout(function(){
      ...
      function addMembers(etext, data) {
        ...
        if (data != null && data.length) {
	      ...
	      if(options.tooltip==true){
		cache.set(xssPrevent(val.key), xssPrevent(val.value), xssPrevent(val["title"]));
		tooltip_hover();
	      }
	      else{
	...
	if (maximum) {
	       ...
	       if(options.tooltip==true){
		  content += '<li title="'+object.title+'" rel="' + object.key + '">' + xssDisplay(itemIllumination(object.value, etext)) + '</li>';
		 tooltip_hover();
	       }else{
      ...
      function bindEvents() {
        ...
        feed.children("li").unbind("mousedown").mousedown( function() {
          var option = $(this);
	  if(options.tooltip==true){
	     addItem(option.text(), option.attr("rel"),option.attr("title"), 0, 0, 1);
	     tooltip_hover();
	  }
          else{
        ...
        maininput.keydown( function(event) {
          ...
          if ((event.keyCode == _key.enter || event.keyCode == _key.tab || event.keyCode == _key.comma) && checkFocusOn()) {
            var option = focuson;
	    if(options.tooltip==true){
	       addItem(option.text(), option.attr("rel"),option.find("span").text(), 0, 0, 1);
	        tooltip_hover();
	    }else{
	       addItem(option.text(), option.attr("rel"), 0, 0, 1);
	    }
      ...
      var cache = {
        'search': function (text, callback) {
	      ...
	      if(options.tooltip==true){
			...
			temp.push({'key': i, 'value': _elem[0], 'title':_elem[1]});
		      }
		    }
	      }
	      else{
