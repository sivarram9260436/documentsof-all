...
(function( $, undefined ) {
  $.fn.fcbkcomplete = function(opt) {
    return this.queue( function() {
      function init() {
        createFCBK();
    if(options.sort==true || options.sort==false ){
      addInput(0);
    }
      }
    ...
    if (options.attachto) {
      ...
      function elPrepare() {
        ...
        $.each(element.children('option'), function(i, option) {
          ...
          tip1 = options.tooltip==true ?  option.attr('title') : '';
          var id = addItem(option.text(), option.val(),tip1, true, option.hasClass("locked"));
          temp_elem.append('<option title="'+tip1+'" value="'+option.val()+'" selected="selected" id="opt_'+id+'"class="selected">'+option.text()+'</option>');
          }
        });
      ...
      /* if(options.tooltip==true){
          addItem(data.title, data.value, data.tip, 0, 0, 0);
       }else{
          addItem(data.title, data.value, 0, 0, 0);
      }*/
      tip2 = options.tooltip==true ? data.tip: '';
       addItem(data.title, data.value, tip2, 0, 0, 0);
        });
        ...
        $(element).bind("selectAll", function(event, data) {
          ...
          $.each($(element).data('cache'), function(key, value,title){
        tip3 = options.tooltip==true?title: '';
            if($.inArray(key, currVals) === -1){
            addItem(value, key, tip3, 0, 0, 0);
            }
        });
            feed.parent().hide()
        });
     ...
     if(options.sort==true){
           var hidden_id = 'autocomplete_pass_'+elemid;
          var set_name = $("#"+elemid).attr('name');
          $("#"+elemid).after('<input type="hidden" name="'+set_name+'" id="'+hidden_id+'" />');
          $('#'+elemid).removeAttr('name');
        //  $("#"+elemid).attr('name',set_name+'_new');
           var ul_holder =  $("#"+elemid).next().next('ul.holder').attr('class');
           var ul_li_input = $('.'+ul_holder).find('li#'+elemid+'_annoninput').attr('id');
           $("."+ul_holder).sortable({
         stop:function(event,ui){
           var sortable_array = [];
          $(this).find('li.bit-box').not('#'+ul_li_input).each(function(){
            var li_id = $(this).attr('id');
            var select_value = parseInt($('#'+elemid+' option#o'+li_id).attr('value'));
             sortable_array.push(select_value);
           });
           
           console.log(sortable_array);
        //  sortable_array = sortable_array.split('undefined')[0];
         // var sor_arr = sortable_array.substring(0,sortable_array.length-1);
         // $("#"+hidden_id).attr('value',"["+sortable_array.join(",")+"]");
           $("#"+hidden_id).attr('value',sortable_array);
         }
          });
        }
      ...
      function addItem(title, value, tip, preadded, locked, focusme) {
    if (!maxItems()) {
          return false;
        }
    ...
    tips = options.tooltip==true?tip: '';
    var li = $('<li title="'+tips+'" class="'+liclass+'" rel="'+value+'" id="pt_'+id+'"></li>').prepend(txt).append(aclose);
        holder.append(li);
   ...
   /* if(options.sort==true){
      ...
      function addMembers(etext, data) {
       ...
       tip1 = options.tooltip==true?xssPrevent(val["title"]): '';
       cache.set(xssPrevent(val.key), xssPrevent(val.value),tip1);
          });
        }
    ...
    if (maximum) {
          ...
          tip2 = options.tooltip==true?object.title: '';
          content += '<li title="'+tip2+'" rel="' + object.key + '">' + xssDisplay(itemIllumination(object.value, etext)) + '</li>';
              counter++;
              maximum--;
            }
          }
      ...
      tip1 = options.tooltip==true?option.attr("title"): '';
      addItem(option.text(), option.attr("rel"),tip1, 0, 0, 1);
          clear_feed(1);
          complete.hide();
        });
       ...
       tip2 = options.tooltip==true?option.find("span").text(): '';
       addItem(option.text(), option.attr("rel"),tip2, 0, 0, 1);
          ...
          if ((event.keyCode == _key.enter || event.keyCode == _key.tab || event.keyCode == _key.comma) && !checkFocusOn()) {
           ...
           if(options.tooltip==true){
          addItem(value, value,value, 0, 0, 1);

        }else{
          addItem(value, value,0, 0, 0, 1);
        }
            ...
            if ((options.addontab || options.addoncomma) && options.newel) {
              focuson = feed.children("li:visible:first");
              var option = focuson;
         tip3 = options.tooltip==true?option.find("span").text(): '';
          addItem(option.text(), option.attr("rel"),tip3, 0, 0, 1);
              return _preventDefault(event);
            }
          }
    ...
    sort:false,
      ...
      var cache = {
        ...
        tip1 = options.tooltip==true?_elem[1]: '';
          if (typeof _elem[0].search === 'function') {
              if (_elem[0].search(regex) != -1) {
            temp.push({'key': i, 'value': _elem[0], 'title':tip1});
              }
          }
        ...
        'set': function (id, val,title) {
          var data = element.data("cache");
    tip2 = options.tooltip==true?title: '';
     data[id] = [val,tip2];
