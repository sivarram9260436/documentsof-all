$('.tiny_contaner .hide_control').live('click',function(){
	var parent_tiny_id = $(this).attr('data-tiny_parentid');
...
function start() {
        ...
        if(jQuery.browser.webkit==true){
                jQuery("#parent_" + tinyid).append('<fieldset style="float:left;width:518px"></fieldset>');
        }
        else{
                jQuery("#parent_" + tinyid).append('<fieldset style="float:left;width:455px"></fieldset>');
        }
        jQuery("#parent_" + tinyid + " fieldset").append('<legend></legend>');
        jQuery("#parent_" + tinyid + " fieldset legend").html("Page " + maxId);
...
function tinyMCEOnDemand(tinyid) {
        tinyMCE.init({
          ...
          /*      theme_advanced_buttons1 : "bold,italic,underline,strikethrough,|,styleselect,formatselect,fontselect,fontsizeselect",
                ...
                theme_advanced_buttons4 : "insertlayer,moveforward,movebackward,absolute,|,styleprops,spellchecker,|,cite,abbr,acronym,del,ins,attribs,|,link,unlink,anchor,image,cleanup,help,code",
                theme_advanced_buttons5 : "visualchars,nonbreaking,template,blockquote,pagebreak,|,insertfile,insertimage,|,forecolor,backcolor,|,fullscreen|,justifyleft,justifycenter,justifyright,justifyfull,|,print,|,ltr,rtl",
               */
                theme_advanced_buttons1:'artsearch,preview,spellchecker,bold,italic,underline,strikethrough,formatselect,separator,justifyleft,justifycenter,justifyright,separator,sub,sup,separator,indent,outdent,separator,bullist,numlist',
                theme_advanced_buttons2:'blockquote,template,image,separator,code,separator,styleprops,link,unlink,separator,undo,redo,separator,fullscreen,example,|,forecolor,backcolor,charmap',
        ...
        $('.inactive_tiny_holder .expand_tinymce').click()
        var scroll_tinymce = 'parent_tinyid_'+$('.inactive_tiny_holder .expand_tinymce:last').attr('data-id');
        $('html, body').animate({ scrollTop: jQuery('#'+scroll_tinymce).offset().top }, 'slow');   
};
...
function mceRemoveControl() {
	    jQuery(".tinyclass").each(function() {
                ...
                console.log(tinyMCE.getInstanceById(id))
