function FileAPI (t, d, f) {
    ...
    this.addFiles = function () {
       ...
       img_count = img_count+this.files.length;
	...
	if (file) {
           ...
           // jQuery(li).append("<p><label>Description:</label><textarea type='text' class='description' ></textarea></p>");
            ...
            jQuery(li).append("<p><label>Credits:</label><input type='text' class='image_credits' /></p>");      
    ...
    var uploadFile = function (file, li) {
        if (li && file) {
          ...
          //  pars = '&image[site_id]=' +document.getElementById("image_site_id").value+'&image[licence_type]=Universal rights granted to all users'+'&commit=Upload Image'+'&multiple_image=true&image[title]='+jQuery('.title').attr('value')+'&image[caption]='+jQuery('.caption').attr('value')+ '&image[image_credits]='+jQuery('.image_credits').attr('value') ;
            pars = '&image[site_id]=' +document.getElementById("image_site_id").value+'&image[licence_type]=Universal rights granted to all users'+'&commit=Upload Image'+'&multiple_image=true&image[title]='+li.getElementsByClassName("title")[0].value+'&image[caption]='+li.getElementsByClassName("caption")[0].value+ '&image[image_credits]='+li.getElementsByClassName("image_credits")[0].value ;
            ...
            xhr.onreadystatechange=function()
            {
               ...
               else if (xhr.readyState == 4 && xhr.status == 200 && single_image == "thumblain")
                {
                    window.location = "http://" + path + "/admin/image_details/save_single_image?id=" + xhr.responseText
                }
         ...
         /*var formData = new FormData(); 
            formData.append("image[title]", jQuery(".title",jQuery(li)).val());
            formData.append("image[caption]", jQuery(".caption",jQuery(li)).val());
