...
function start() {
    ...
    tinymce.execCommand('mceAddEditor', true, "#parent_" + tinyid);
//    setTimeout(tinyMCEOnDemand(tinyid), 1000)
    jQuery("#parent_" + tinyid + " .expand_tinymce").click();
...
function tinyMCEOnDemand(tinyid, width, height) {
    tinymce.init({
        ...
        plugins: [
            "powerpaste kreimage kredigitalassets advlist autolink autosave link lists charmap preview hr anchor image image imagetools",
        ...
        toolbar1: "bold italic underline strikethrough pastetext searchreplace styleselect formatselect | bullist numlist | blockquote  undo redo table | media kredigitalassets kreimage code | autolink link unlink fullscreen | preview tinymcespellchecker hr charmap removeformat visualchars",
        menubar: false,
        content_style: "div.left{float:left;}div.right{float:right;}div.center{text-align:center;margin: 0 auto;}",
...
function mceRemoveControl(id) {
    //jQuery(".tinyclass").each(function() {
    //  var id = jQuery(this).attr("id");
    if(tinyMCE.get(id)) {
        tinyMCE.execCommand("mceRemoveEditor", true,"#parent_" + tinyid);
